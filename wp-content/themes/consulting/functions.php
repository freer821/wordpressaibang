...
if (!function_exists('consulting_load_theme_scripts_and_styles')) {
    function consulting_load_theme_scripts_and_styles()
    {
        ...
        if (!is_admin()) {
            ...
            $consulting_config = consulting_config();
            if (stm_check_layout('layout_13') || stm_check_layout('layout_14') || stm_check_layout('layout_15') || stm_check_layout('layout_san_francisco')) {
...
if (is_super_admin()) {
    add_action('init', 'consulting_update_layout');

    function consulting_update_layout()
    {
        if (isset($_GET['stm_update_layout'])) {
            $layout = $_GET['stm_update_layout'];
            update_option('consulting_layout', '');
            echo 'layout updated';
            die;
        }
    }
